...
- Final MP4 can be produced from the CLI with `python -m bworkflow_sql render-final-video <project_id> --account <账号> --product-media-mode video_preferred`. This keeps B-Workflow as the orchestration layer, generates a `final_mp4` RenderPackage, calls CutMe once through `CutMeAdapter`, verifies the MP4, writes a run manifest under `data/workspace/project-<id>/runs/`, and returns `output_mp4_link` plus `run_manifest_path` for direct opening/recovery. CutMe privately owns source validation, job-local preparation, and fast-final rendering behind its versioned machine result; B-Workflow must not reconstruct those commands. To prepend an accepted intro MP4, add `--intro-video <intro.mp4> --full-output <full.mp4>`; the concat stage standardizes the complete MP4 to H.264 1920x1080/30fps and AAC 48kHz. Use `--acceptance-mode quick` for fast delivery checks, `visual` when key-frame inspection is needed, and `full` for loudnorm plus visual archival verification. For production delivery, prefer `--delivery-dir <dir>`: `完整成片-<timestamp>.mp4` and `商品推荐段-<timestamp>.mp4` are written directly under that directory, while evidence and process files go under `02_验收证据\<timestamp>\` and `03_过程记录\<timestamp>\`.
