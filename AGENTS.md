...
- 当前默认使用方式是由 Agent/Skill 调用本仓库的 CLI 和底层服务，再调用 CutMe/Remotion 完成自动剪辑；桌面客户端界面不再是默认交付范围。
- 后续功能、修复、模板或工作流需求，不要为了在客户端中展示或同步该能力而修改桌面页面、弹窗、按钮、表单、客户端状态、GUI 打包或 GUI 测试。
- Skill/Agent 工作流依赖的 CLI、服务层、SQLite/数据契约、素材处理、配音对齐、RenderPackage 组装和 CutMe 适配仍是正式维护范围，可以按需求修改和验证。
- 底层或 CLI 发生变化时，默认不要求兼容桌面客户端。只有用户在当前任务中明确要求修改客户端界面，才进入客户端代码并执行相应 GUI 验证。

## 跨仓库测试产物清理

- 用户说“清理无关的测试文件”时，不在本仓库按名称或年龄直接清理。先回到
  `G:\workspace\Bilibili-TotalControl`，读取
  `docs/TEST_ARTIFACT_GOVERNANCE.md` 并运行
  `scripts\test-artifact-cleanup.ps1` 生成只读建议。
- 旧文件只有在当次任务展示精确保留/迁移/删除候选并获得用户明确确认后才能处理；
  B-Workflow 资源删除继续使用自己的 `resource-cleanup-plan/delete` 和新鲜指纹复核。
...
| 多任务身份 | 每个新任务必须使用 schema-v2 pipeline 和唯一 `episode_id`；新 run manifest、配方、验收产物、口播稿快照和 `production_runs` 都按 episode 归属。旧 pipeline/空 `episode_id` 记录只作历史，不迁移、不参与新流程。 |
| 正式渲染占用 | 引言、完整成片和生产重渲染共用 `data\workspace\.locks\production-render.lock`。遇到 `render_busy` 必须立即返回当前 owner；禁止排队、循环重试或绕过 B-Workflow 直调 CutMe。预检、文案、配音和打包准备不占锁。 |
...
| 产物确认凭证 | 引言验收使用 `confirm-intro-video`，正式成片使用 `confirm-production`。两者都向 schema-v2 `.pipeline.json` 写入绑定路径、SHA-256、大小、来源版本和确认时间的 `artifact_approvals`；文件变化后旧确认失效，run manifest 与 pipeline 的 `episode_id` 不一致时拒绝确认。 |
| 发布完成与归档 | Master 的 `published`/B站链接只证明远端发布，不能替代本地归档。复用 `production_runs` 和 `.pipeline.json` 的 `phases.publishing`；`complete-publishing` 默认移动到 `G:\2026项目-b站\已发布视频` 下已存在的当前月份目录，不存在则放根目录且不建月份目录；`--archive-dir` 可覆盖，`--current-path` 校验手工移动后的文件。成功并核验归档路径后才进入 `done`。 |
| 蓝链回流 | 发布归档后的显式可选动作，默认不执行、不阻塞本期完成。只有用户明确要求时才创建或继续 `phases.blue_link_backfill`；已开始且为 `partial` 的回流仍保持可操作。`publishing-context` 只通过正式成片的本地 `account_id` 读取固定 Master UUID/B站 MID/方案 ID；`resolve-blue-link-backfill <backfill_id> --workspace-id <uuid>` 依次触发无 ID 行的严格/模糊标题候选，再只租约无候选或已拒绝的浏览器行，每条只尝试一次并立即回写。`resolve-blue-links` 仅作单链接诊断。标题决定必须带扫描版本和稳定批次 ID；`stored_slot_conflict` 会整批回滚，需保留旧槽位、排除冲突项并用新批次 ID 提交其余项；最高分并列必须等待用户指定 UID。京东请求默认限速，明确 403 后持久熔断两小时且不阻塞淘宝；库内缺商品、旧链冲突、延后和安全挂起行不会重复打开。未完成汇报必须读取 Master 持久化分组，并为每类给出最多三条可点击源链接。`record-blue-link-backfill` 必须读取 Master 快照核验身份和计数后再写同一 `production_runs` 行。 |
...
| 验证命令 | 从仓库根目录运行 `python -m pytest tests`，不要裸跑无路径的 pytest；`data/workspace/manual-tests` 可能保留外部源码副本及其 tests，不属于本仓测试收集范围。最小回归常用：`python -m pytest -q tests/test_workflow_service.py tests/test_ui_helpers.py tests/test_repositories.py tests/test_sync_service.py`。 |
